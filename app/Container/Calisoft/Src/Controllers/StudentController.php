...
use App\Container\Calisoft\Src\Requests\ItemsCodificacion;
...
class StudentController extends Controller
{
    ...
    public function resultados(Proyecto $proyect)
    {
        ...
        return view('calisoft.student.student-resultados', compact('modelado','codificacion','plataforma','resultadoBD','total','nombrePj'));
    }
    
    public function resultadosGraficas(){
        ...
        return view('calisoft.student.student-resultadosGenerales-graficas',compact('modelado','codificacion','plataforma','resultadoBD','total','nombrePj','promedioGeneralDocumentos','promedioGeneralCasos','promedioGeneralCodificacion','promedioGeneralBD','promedioGeneralTotal'));
    }
    public function resultadoGeneral(){
        ...
        try{
            ...
            $plataformaPorcentaje = round($plataformaPorcentaje->avg());
        ...
        $modeladoTabla = $pj->documentos()->with('tipo', 'evaluaciones.componente', 'evaluaciones.evaluador')->get();
        $modeladoTabla = (new Calificaciones($pj))->modelacion();
        $modeladoTabla = (new Calificaciones($pj))->modelacion();
        if($modeladoTabla == null){
            $modeladoTabla = 0;
        }
        if($modeladoTabla >= 100){
            $modeladoTabla = 100;
        }

        $codificacionTabla = array();
        $scriptIdTabla = DB::table('TBL_Scripts')->where('TBL_Scripts.FK_ProyectoId',$proyecto)->pluck('PK_id');
        foreach ($scriptIdTabla as $script) {
            $codificacionTabla = round((DB::table('TBL_NotaCodificacion')->where('TBL_NotaCodificacion.FK_ScriptsId',$script)->sum('nota'))*15.3);
            if($codificacionTabla >= 100){
                $codificacionTabla = 100;
            }
        }
        if($codificacionTabla == null){
            $codificacionTabla = 0;
        }

        try{
            $plataformaTabla = CasoPrueba::where('FK_ProyectoId',$proyecto)->pluck('calificacion');
            $plataformaTabla = round($plataformaTabla->avg());
            if($plataformaTabla >= 100){
                $plataformaTabla = 100;
            }
        }catch(Exception $e){
            $plataformaTabla = 0;
        ...
        $idSqlTabla = ArchivoSql::where('FK_ProyectoId',$proyecto)->pluck('PK_id');
        if((count($idSqlTabla) >= 1)){
            $contadorBDTabla = calificacionBD::where('calificacion','>',0)->where('FK_ArchivoBdId',$idSqlTabla)->pluck('calificacion');
            $mediador = 0;
            $counter = 0;
            ...
            if(($counter)== 0){
                $interventor = 0;
                $resultadoBDTabla = round($interventor*20);
            }else{
                $interventor = $mediador/$counter;
                $resultadoBDTabla = round($interventor*20);
            }
        ...
        return view('calisoft.student.student-porcentaje-general', compact('modeladoPorcentaje',
        'codificacionPorcentaje','plataformaPorcentaje','resultadoBDPorcentaje','totalPorcentaje',
        'nombrePj','promedioGeneralDocumentosGrafico','promedioGeneralCasosGrafico',
        'promedioGeneralCodificacionGrafico','promedioGeneralBDGrafico','promedioGeneralTotalGrafico'
        ,'modeladoGrafico','codificacionGrafico','plataformaGrafico','resultadoBDGrafico','totalGrafico',
        'promedioGeneralDocumentosTabla','promedioGeneralCasosTabla','promedioGeneralCodificacionTabla',
        'promedioGeneralBDTabla','promedioGeneralTotalTabla','modeladoTabla','codificacionTabla',
        'plataformaTabla','resultadoBDTabla','totalTabla'));
    }

    public function resultadosEspecificos()
    {
        $proyecto = auth()->user()->proyectos()->first();
        $id = $proyecto->PK_id;
        ...
        return view('calisoft.student.student-resultado-especifico',
             compact('proyecto','documentos','categoria','modelado','codificacion','notaCodificacion','plataforma',
             'notaBd','resultadoBd'));
    }
    public function sugerencias(){
        ...
        $modeladoFaltante = Documento::
            ...
            ->get();
        $modelado = $proyecto->documentos()->with('tipo','evaluaciones')->get();
        ...
        foreach($codificacion as $scrip){
            if($scrip->nota <= $minScript){
                ...
                if($scrip->nota == null || $scrip->nota == 0){
                    $lessCodificacion = null;
                }
        ...
        return view('calisoft.student.student-resultado-sugerencia',
